-- Repair active system-category children left under duplicate parents that were
-- already soft-deleted before the canonical-parent dedupe logic existed.
...
CREATE TEMP TABLE deleted_system_parent_repair_map ON COMMIT DROP AS
WITH root_parent_repairs AS (
  SELECT
    deleted_parent.id AS deleted_id,
    active_parent.id AS canonical_id
  FROM public.categories AS deleted_parent
  INNER JOIN public.categories AS active_parent
    ON active_parent.user_id IS NULL
    AND active_parent.is_system = true
    AND active_parent.deleted = false
    AND active_parent.parent_id IS NULL
    AND active_parent.system_name = deleted_parent.system_name
    AND active_parent.level = deleted_parent.level
    AND active_parent.type IS NOT DISTINCT FROM deleted_parent.type
  WHERE deleted_parent.user_id IS NULL
    AND deleted_parent.is_system = true
    AND deleted_parent.deleted = true
    AND deleted_parent.parent_id IS NULL
),
level2_parent_repairs AS (
  SELECT
    deleted_parent.id AS deleted_id,
    active_parent.id AS canonical_id
  FROM public.categories AS deleted_parent
  LEFT JOIN root_parent_repairs AS deleted_root_parent
    ON deleted_root_parent.deleted_id = deleted_parent.parent_id
  INNER JOIN public.categories AS active_parent
    ON active_parent.user_id IS NULL
    AND active_parent.is_system = true
    AND active_parent.deleted = false
    AND active_parent.parent_id = coalesce(
      deleted_root_parent.canonical_id,
  ...
  WHERE deleted_parent.user_id IS NULL
    AND deleted_parent.is_system = true
    AND deleted_parent.deleted = true
    AND deleted_parent.parent_id IS NOT NULL
    AND deleted_parent.level = 2
),
level3_parent_repairs AS (
  SELECT
    deleted_parent.id AS deleted_id,
    active_parent.id AS canonical_id
  FROM public.categories AS deleted_parent
  LEFT JOIN level2_parent_repairs AS deleted_level2_parent
    ON deleted_level2_parent.deleted_id = deleted_parent.parent_id
  INNER JOIN public.categories AS active_parent
    ON active_parent.user_id IS NULL
    AND active_parent.is_system = true
    AND active_parent.deleted = false
    AND active_parent.parent_id = coalesce(
      deleted_level2_parent.canonical_id,
      deleted_parent.parent_id
    )
    AND active_parent.system_name = deleted_parent.system_name
    AND active_parent.level = deleted_parent.level
  ...
  WHERE deleted_parent.user_id IS NULL
    ...
    AND deleted_parent.deleted = true
    AND deleted_parent.parent_id IS NOT NULL
    AND deleted_parent.level = 3
)
SELECT DISTINCT deleted_id, canonical_id
FROM root_parent_repairs
UNION
SELECT DISTINCT deleted_id, canonical_id
FROM level2_parent_repairs
UNION
SELECT DISTINCT deleted_id, canonical_id
FROM level3_parent_repairs;

UPDATE public.categories AS child_categories
SET
  parent_id = deleted_parent_map.canonical_id,
  updated_at = now()
FROM deleted_system_parent_repair_map AS deleted_parent_map
WHERE child_categories.parent_id = deleted_parent_map.deleted_id
  AND child_categories.deleted = false
  AND child_categories.parent_id <> deleted_parent_map.canonical_id;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_categories_unique_active_custom_root
  ON public.categories (user_id, display_name, type)
  WHERE deleted = false
    AND is_system = false
