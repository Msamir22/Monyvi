...
| Known bank/wallet picker and internal sender defaults                  | `InstitutionPicker.test.tsx`, `SenderChipsField.test.tsx`, `InstitutionProviderSection.test.tsx`, `useAccountForm.test.ts`, `useEditAccountForm.test.ts` | Maestro flow exists; latest local run blocked by runner/emulator preflight |
| Other/manual bank and wallet providers                                 | `InstitutionProviderSection.test.tsx`, `account-service.test.ts`, `AccountCard.test.tsx`                                                                 | Maestro flow exists; manual validation covered                             |
| Provider-aware uniqueness                                              | `account-service.test.ts`, `edit-account-service.test.ts`, `useAccountForm.test.ts`                                                                      | Manual timing/check covered                                                |
| SMS sender matching for banks and wallets                              | `sms-account-matcher.test.ts`                                                                                                                            | Existing live/batch fingerprint tests                                      |
| Explainer tooltip and localization                                     | `InstitutionProviderSection.test.tsx`, `npm run i18n:check -w @monyvi/mobile`                                                                            | App-route UI tests                                                         |
