...
| MT-002 | Needs retest                   | Previous run created `Manual 027 CIB` from the Bank dropdown and confirmed CIB logo/account surfaces. This needs retest after the approved behavior change: registry sender names are configured internally for matching, and the visible sender chips field starts empty for custom/extra sender names only.                                                                                                                                                                     |
...
| MT-008 | Needs retest                   | Previous run confirmed compact Monyvi-owned product UI that opens without leaving the screen and closes when tapping outside. This needs retest after the approved copy change: `Used only to match SMS alerts. No PINs or passwords needed.`                                                                                                                                                                                                                                     |
...
- `npx jest --runInBand --detectOpenHandles --forceExit __tests__/components/add-account/InstitutionProviderSection.test.tsx __tests__/app/add-account-institution-info.test.tsx __tests__/app/edit-account-institution-info.test.tsx __tests__/hooks/useEditAccountForm.test.ts`
- `npm run lint -w @monyvi/mobile`
- `npm run db:migrate`
- `npx jest --runInBand --detectOpenHandles --forceExit __tests__/services/sms-account-matcher.test.ts __tests__/services/sms-account-resolver.test.ts __tests__/services/edit-account-service.test.ts __tests__/services/account-service.test.ts __tests__/validation/account-validation.test.ts __tests__/hooks/useEgyptianInstitutionEligibility.test.ts __tests__/hooks/useAccountForm.test.ts __tests__/components/add-account/SenderChipsField.test.tsx __tests__/components/add-account/InstitutionPicker.test.tsx __tests__/components/add-account/InstitutionProviderSection.test.tsx __tests__/app/add-account-institution-info.test.tsx __tests__/app/edit-account-institution-info.test.tsx __tests__/components/accounts/AccountCard.test.tsx __tests__/components/dashboard/AccountsSection.test.tsx __tests__/migrations/account-institution-senders-migration.test.ts __tests__/utils/account-institution-presentation.test.ts __tests__/scripts/manual-qa-seed.test.ts`
